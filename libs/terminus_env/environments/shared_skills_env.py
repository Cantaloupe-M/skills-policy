...
class SharedSkillsDockerEnvironment(DockerEnvironment):
    ...
    def __init__(
        ...
        *args,
        ...
        base_image_from: str = "skillflow/harbor-cli-base:ubuntu24.04:ubuntu24.04",
        llm_adapter_network: str | None = None,
        enable_host_gateway: bool = False,
        **kwargs,
    ):
        self._project_template_dir = self._resolve_host_path(project_template_dir)
        self._shared_skill_mount_targets = self._normalize_mount_targets(mount_targets)
        ...
        self._llm_adapter_network = llm_adapter_network
        self._enable_host_gateway = enable_host_gateway
    ...
    def _docker_compose_paths(self) -> list[Path]:
        ...
        return [*super()._docker_compose_paths, custom_compose_path]

    async def prepare_logs_for_host(self) -> None:
        """Chown bind-mounted logs, artifacts, and shared skills back to host UID.

        Tasks should run with their normal container user so paths like /root in
        benchmark prompts keep working.  Before Docker teardown, make the host
        copies readable/deletable for the user who launched SkillFlow.
        """
        await super().prepare_logs_for_host()
        if self._is_windows_container:
            return

        for mount_target in self._shared_skill_mount_targets:
            try:
                await self._chown_to_host_user(mount_target, recursive=True)
            except Exception as exc:
                self.logger.warning("Failed to chown shared skills mount %s: %s", mount_target, exc)
    ...
    def _write_shared_skills_override(self, target_compose_path: Path) -> None:
        ...
        # Preserve base compose mounts that would otherwise be replaced by
        # Docker Compose list-merge semantics.  Without these, agent logs,
        # verifier output, and artifacts are invisible on the host until the
        # trial ends and Harbor explicitly copies them out.
        host_agent_logs = self.trial_paths.agent_dir.resolve().as_posix()
        host_verifier_logs = self.trial_paths.verifier_dir.resolve().as_posix()
        host_artifacts = self.trial_paths.artifacts_dir.resolve().as_posix()
        volumes.extend([
            f"{host_agent_logs}:/logs/agent",
            f"{host_verifier_logs}:/logs/verifier",
            f"{host_artifacts}:/logs/artifacts",
        ])
        ...
        if self._enable_host_gateway:
            compose_data["services"]["main"]["extra_hosts"] = [
                "host.docker.internal:host-gateway"
            ]
        if self._llm_adapter_network:
            compose_data["services"]["main"]["networks"] = ["default", "llm-adapter"]
            compose_data["networks"] = {
                "llm-adapter": {
                    "external": True,
                    "name": self._llm_adapter_network,
                }
            }
