...
def compute_expected_snack(input_root: Path):
    """Compute expected outputs."""

    wb = load_workbook(input_root / 'Snack_DC_August_Balancing.xlsx', data_only=True)
    stock_ws = wb['Stock Snapshot']
    inb_ws = wb['Scheduled Inbounds']
    cfg_ws = wb['Load Config']
    ...
    # Inbounds aggregation
    inbound_by_sku = {}
    for r in range(2, inb_ws.max_row + 1):
        sku = normalize_text(inb_ws.cell(row=r, column=1).value)
        if not sku:
            continue
        d = parse_date(inb_ws.cell(row=r, column=2).value)
        if d is None:
            continue
        cases = to_number(inb_ws.cell(row=r, column=3).value)
...
def read_output_snack(output_file: Path):
    wb = load_workbook(output_file, data_only=True)
    assert set(wb.sheetnames) == set(['Load_Detail', 'Load_Action_Summary']), f'Sheet names mismatch: {wb.sheetnames}'
    detail_ws = wb['Load_Detail']
    action_ws = wb['Load_Action_Summary']
    ...
    assert set(action_header) == set(['Item_Code', 'Required_Delivery_Date', 'Pallets_Required', 'Additional_Cases_Needed', 'Earlier_Delivery_Required']), f'Action header mismatch: {action_header}'
    action_map = {}
...
def main():
    ...
    output_file = Path('/root/snack_dc_august_load_plan.xlsx') if in_harness else (task_dir / 'snack_dc_august_load_plan.xlsx')

    assert output_file.exists(), f'Output file not found: {output_file}'

    expected = compute_expected_snack(input_root)
    got = read_output_snack(output_file)
    ...
    for key in expected['detail_order']:
        ...
        for field in ['On_Floor_Cases', 'Daily_Sales_Cases_Per_Day', 'Current_Days_On_Hand', 'Inbound_Cases_By_Horizon', 'Delivered_Days_On_Hand', 'Remaining_Demand_Cases', 'Additional_Cases_Needed', 'Pallets_Required']:
