...
function readOutputRows(outputFile) {
  ...
  assert(
    [...wb.SheetNames].sort().join(',') === ["SKU_Results", "Additional_Shipments_Needed"].sort().join(','),
    `Sheet names must contain SKU_Results and Additional_Shipments_Needed`
  );
...
function main() {
  ...
  assert.strictEqual(String(skuRows[0][0]).toUpperCase(), "FIELD", "SKU_Results A1 must be Field");
  assert.strictEqual(String(skuRows[0][1]).toUpperCase(), "VALUE", "SKU_Results B1 must be Value");
  assert.strictEqual(String(skuRows[1][0]).toUpperCase(), "ASOFDATE", "SKU_Results A2 label mismatch");
  assert.strictEqual(toIsoDate(skuRows[1][1]), expected.asOfDate, "AsOfDate mismatch");
  assert.strictEqual(
    String(skuRows[2][0]).toUpperCase(),
    "PLANNINGHORIZONEND",
  ...
  assert.strictEqual(
    String(skuRows[3][0]).toUpperCase(),
    "REMAININGDAYSINJULY",
  ...
  assert(
    [...(skuRows[5] || []).slice(0, requiredSkuHeaders.length)].sort().join(',') === [...requiredSkuHeaders].sort().join(','),
    "SKU_Results headers must contain required columns"
  );
  ...
  assert(
    [...outputOrder].sort().join(',') === [...expectedOrder].sort().join(','),
    "SKU row keys must match"
  );
  ...
  assert(
    [...(additionalRows[0] || []).slice(0, requiredAdditionalHeaders.length)].sort().join(',') === [...requiredAdditionalHeaders].sort().join(','),
    "Additional_Shipments_Needed headers must contain required columns"
  );
  ...
  assert(
    [...outputAdditionalOrder].sort().join(',') === [...expectedAdditionalOrder].sort().join(','),
    "Additional sheet row keys must match"
  );
