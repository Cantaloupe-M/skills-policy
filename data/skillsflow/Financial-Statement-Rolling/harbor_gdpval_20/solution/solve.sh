...
if [ -f /root/prepaid_expenses_schedule_input.csv ]; then
  DATA_ROOT="/root"
  OUTPUT_FILE="/root/Aurisic_Amortization_4-25.xlsx"
else
  DATA_ROOT="${TASK_DIR}/environment"
  OUTPUT_FILE="${TASK_DIR}/Aurisic_Amortization_4-25.xlsx"
fi
...
function buildSummarySheet(expInfo, insInfo) {
  const ws = {};
  setCell(ws, "A1", "Aurisic");
  setCell(ws, "A2", "Prepaid Expenses - Summary");
  setCell(ws, "A3", "For the period: 1/1/2025 - 4/30/2025");

  setCell(ws, "A5", "Description");
  setCell(ws, "B5", "Amount");

  setCell(ws, "A6", "Prepaid Expenses (Acct 1250)");
  setCell(ws, "A7", "PPD Expenses - 2025 Total");
  setCell(ws, "B7", expInfo.totalAdds, `'PPD Exp #1250'!O${expInfo.totalsRow}`);
  setCell(ws, "A8", "PPD Expenses - Amortized in 2025");
  setCell(ws, "B8", expInfo.totalAmort, `'PPD Exp #1250'!O${expInfo.endingRow}`);
  setCell(ws, "A9", "PPD Expenses - GL Balance - 4/30/2025");
  setCell(ws, "B9", expInfo.endingBalance, `'PPD Exp #1250'!O${expInfo.glRow}`);

  setCell(ws, "A11", "Prepaid Insurance (Acct 1251)");
  setCell(ws, "A12", "PPD Insurance - 2025 Total");
  setCell(ws, "B12", insInfo.totalAdds, `'PPD Ins #1251'!O${insInfo.totalsRow}`);
  setCell(ws, "A13", "PPD Insurance - Amortized in 2025");
  setCell(ws, "B13", insInfo.totalAmort, `'PPD Ins #1251'!O${insInfo.endingRow}`);
  setCell(ws, "A14", "PPD Insurance - 2025 GL Balance - 4/30/2025");
  setCell(ws, "B14", insInfo.endingBalance, `'PPD Ins #1251'!O${insInfo.glRow}`);
...
const glPath = path.join(dataRoot, "gl_balances.json");
const expPath = path.join(dataRoot, "prepaid_expenses_schedule_input.csv");
const insPath = path.join(dataRoot, "prepaid_insurance_schedule_input.csv");
...
const expDetail = buildDetailSheet(
  "PPD Exp #1250",
  "Prepaid Exp Account #1250 as of 4/30/25",
  expRows,
...
const insDetail = buildDetailSheet(
  "PPD Ins #1251",
  "Prepaid Insurance Account #1251 as of 4/30/25",
  insRows,
...
XLSX.utils.book_append_sheet(wb, summary, "Prepaid Summary");
XLSX.utils.book_append_sheet(wb, expDetail.ws, "PPD Exp #1250");
XLSX.utils.book_append_sheet(wb, insDetail.ws, "PPD Ins #1251");
