...
const runningInHarbor = fs.existsSync("/root/prepaid_expenses_schedule_input.csv");
...
const outputFile = runningInHarbor
  ...
  : path.join(localTaskDir, "Aurisic_Amortization_4-25.xlsx");
...
const expectedSheets = ["Prepaid Summary", "PPD Exp #1250", "PPD Ins #1251"];
assert(
...
const expenseRows = readCsv(path.join(dataRoot, "prepaid_expenses_schedule_input.csv"));
const insuranceRows = readCsv(path.join(dataRoot, "prepaid_insurance_schedule_input.csv"));
const gl = JSON.parse(fs.readFileSync(path.join(dataRoot, "gl_balances.json"), "utf8"));

function validateDetailSheet(sheetName, rows, glValues, titleContains) {
  const ws = workbook.Sheets[sheetName];
  assert(ws, `${sheetName} sheet missing`);

  assert(text(value(ws, "A1")) === "Aurisic", `${sheetName}!A1 must be Aurisic`);
  assert(
...
const insRowsInfo = validateDetailSheet(
  "PPD Ins #1251",
  insuranceRows,
  gl.prepaid_insurance_1251,
  "1251"
);

const summary = workbook.Sheets["Prepaid Summary"];
assert(summary, "Prepaid Summary sheet missing");

assert(text(value(summary, "A1")) === "Aurisic", "Prepaid Summary!A1 must be Aurisic");
assert(
  text(value(summary, "A2")).toLowerCase().includes("summary"),
  "Prepaid Summary!A2 must include Summary"
);
assert(
  text(value(summary, "A3")).includes("4/30/2025"),
  "Prepaid Summary!A3 must include period through 4/30/2025"
);

assert(text(value(summary, "A5")) === "Description", "Prepaid Summary!A5 must be Description");
assert(text(value(summary, "B5")) === "Amount", "Prepaid Summary!B5 must be Amount");

assert(
  normalizeFormula(formula(summary, "B7")) ===
    normalizeFormula(`'PPD Exp #1250'!O${expRowsInfo.totalsRow}`),
  "Prepaid Summary!B7 formula mismatch"
);
assert(
  normalizeFormula(formula(summary, "B8")) ===
    normalizeFormula(`'PPD Exp #1250'!O${expRowsInfo.endingRow}`),
  "Prepaid Summary!B8 formula mismatch"
);
assert(
  normalizeFormula(formula(summary, "B9")) ===
    normalizeFormula(`'PPD Exp #1250'!O${expRowsInfo.glRow}`),
  "Prepaid Summary!B9 formula mismatch"
);
assert(
  normalizeFormula(formula(summary, "B12")) ===
    normalizeFormula(`'PPD Ins #1251'!O${insRowsInfo.totalsRow}`),
  "Prepaid Summary!B12 formula mismatch"
);
assert(
  normalizeFormula(formula(summary, "B13")) ===
    normalizeFormula(`'PPD Ins #1251'!O${insRowsInfo.endingRow}`),
  "Prepaid Summary!B13 formula mismatch"
);
assert(
  normalizeFormula(formula(summary, "B14")) ===
    normalizeFormula(`'PPD Ins #1251'!O${insRowsInfo.glRow}`),
  "Prepaid Summary!B14 formula mismatch"
);
assert(
  normalizeFormula(formula(summary, "B16")) === normalizeFormula("B9+B14"),
  "Prepaid Summary!B16 formula must be B9+B14"
);
