...
def ensure_direct_child(parent: ET.Element, prefix: str, tag: str) -> ET.Element:
...
def adjust_shape_position(shape: ET.Element, slide_width: int, slide_height: int, style: dict[str, object]) -> None:
    sp_pr = ensure_direct_child(shape, "p", "spPr")
...
def find_caption_paragraph(root: ET.Element, raw_caption: str) -> ET.Element:
    ...
    for paragraph in root.findall(".//a:p", NS):
        text = normalize_text("".join(node.text or "" for node in paragraph.findall(".//a:t", NS)))
        if text == target:
            return paragraph
...
def load_alias_map(config: dict[str, object]) -> dict[str, str]:
    ...
    with (Path("/root") / str(alias_file)).open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            mapping[normalize_text(row["raw"])] = normalize_text(row["canonical"])
    return mapping
...
def ordered_clean_captions(config: dict[str, object], alias_map: dict[str, str]) -> list[str]:
    ...
    for slide_cfg in config["slides"]:
        ...
        clean = normalize_text(alias_map.get(raw, str(slide_cfg["caption_clean"])))
...
def append_summary_slide(unpack_dir: Path, title: str, bullets: list[str]) -> None:
    ...
    slide_files = sorted(slides_dir.glob("slide*.xml"), key=slide_number)
...
def main() -> None:
    ...
    for slide_cfg in config["slides"]:
        ...
        raw = normalize_text(str(slide_cfg["caption_raw"]))
        clean_text = normalize_text(alias_map.get(raw, str(slide_cfg["caption_clean"])))
        rewrite_caption_paragraph(paragraph, clean_text, config["style"])

        shape = find_parent_shape(paragraph, build_parent_map(root))
        if shape is None:
