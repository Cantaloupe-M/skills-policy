...
class FilingToolkit:
    ...
    def fund_snapshot(self, quarter, accession_number):
        ...
        return {"all_rows": subset, "stock_rows": stock_rows, "grouped": grouped, "aum": float(subset["VALUE"].sum())}
    ...
    def top_holders(self, quarter, cusip, topk):
        ...
        for _, row in agg.iterrows():
            ...
            rows.append({"accession_number": row["ACCESSION_NUMBER"], "manager_name": manager, "value": float(row["VALUE"])})
        return rows
