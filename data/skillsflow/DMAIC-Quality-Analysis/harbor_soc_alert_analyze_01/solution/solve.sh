...
INPUT_CSV = Path("/root/soc_alert_data.csv")
OUT_JSON = Path("/root/soc_analyze_metrics.json")
OUT_MD = Path("/root/soc_analyze_brief.md")
...
TARGET = 560.0
BASELINE = 500.0
METRIC_COL = "ResolvedAlerts"
METRIC_LABEL = "resolved alerts"
...
def main() -> None:
    ...
    payload = {
        "source_file": "soc_alert_data.csv",
        "filters": {
    ...
    md = f"""# SOC Analyze Tollgate Brief

## Project Charter
- Domain objective: improve resolved alerts under a DMAIC Analyze gate.
- Baseline value: {BASELINE:.0f}
- Target value: {TARGET:.0f}
- Current mean value (business days, 2025-01-04..2025-03-01): {mean_value:.2f}
- Operational impacts: slower threat containment, analyst overtime spikes, delayed incident closure, handoff backlog growth.
...
1. Finalize Analyze package and leadership readout (Owner: SOC Analytics Lead, Due: 2025-03-14)
2. Launch weekly monitoring and exception review cadence (Owner: Incident Response Manager, Due: 2025-03-24)
"""
