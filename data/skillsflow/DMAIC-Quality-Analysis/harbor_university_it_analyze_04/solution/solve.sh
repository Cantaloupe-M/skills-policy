...
INPUT_CSV = Path("/root/it_helpdesk_data.csv")
OUT_JSON = Path("/root/it_helpdesk_analyze_metrics.json")
OUT_MD = Path("/root/it_helpdesk_analyze_brief.md")
...
TARGET = 300.0
BASELINE = 260.0
METRIC_COL = "ResolvedStudentTickets"
METRIC_LABEL = "resolved student tickets"
...
def main() -> None:
    ...
    payload = {
        "source_file": "it_helpdesk_data.csv",
        "filters": {
    ...
    md = f"""# Campus IT Helpdesk Analyze Tollgate Brief

## Project Charter
- Domain objective: improve resolved student tickets under a DMAIC Analyze gate.
- Baseline value: {BASELINE:.0f}
- Target value: {TARGET:.0f}
- Current mean value (business days, 2025-01-04..2025-03-01): {mean_value:.2f}
- Operational impacts: longer student service wait times, escalation backlog for advisors, higher seasonal staffing pressure, missed onboarding support handoffs.
...
1. Finalize Analyze package and leadership readout (Owner: IT Service Desk Lead, Due: 2025-03-16)
2. Launch weekly monitoring and exception review cadence (Owner: Student Technology Director, Due: 2025-03-27)
"""
